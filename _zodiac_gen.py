...
<p>그래도 이 페이지를 만들어 둔 이유는 두 가지입니다. 하나는 흔한 띠별 운세가 열두 개의 글을 돌려 쓰는 데 비해 여기는 <strong>오늘의 일진을 실제로 계산해</strong> 관계를 뽑는다는 것이고, 다른 하나는 그 산식을 숨기지 않고 전부 적어 두었다는 것입니다. 어떻게 나온 숫자인지 알면 그 숫자를 어디까지 믿을지도 스스로 정할 수 있습니다. 더 촘촘한 결과가 필요하면 <a href="/fortune/today/">생년월일로 보는 오늘의 운세</a>가 있습니다. 그쪽은 열두 칸이 아니라 60일주와 오늘 천간 열 가지가 곱해진 격자를 씁니다. 사주가 아니라 서양 점성술 쪽이 궁금하다면 <a href="/fortune/today/horoscope/aries/">별자리 오늘의 운세</a>도 같은 방식으로 산식을 공개해 두었습니다.</p>
