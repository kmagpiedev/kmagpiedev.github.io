...
export class GenerativeAudio {
  ...
  createGraph() {
    ...
    const droneFrequencies = [55, 82.5, 110];
  ...
  update(timelineTime, state) {
    ...
    this.droneFilter.frequency.setTargetAtTime(230 + state.rift * 430 + state.seal * 310, now, .14);
    this.droneGain.gain.setTargetAtTime(.11 + state.window * .035 + state.rift * .10 + state.seal * .055, now, .17);
    this.noiseFilter.frequency.setTargetAtTime(360 + state.rift * 1450 + state.lotus * 620, now, .12);
    this.noiseGain.gain.setTargetAtTime(.008 + state.rift * .052 + state.seal * .013, now, .14);
    this.shimmer.frequency.setTargetAtTime(392 + state.seal * 392 + Math.sin(timelineTime * .22) * 7, now, .18);
    this.shimmerGain.gain.setTargetAtTime(state.seal * .035 + state.lotus * .022, now, .20);
