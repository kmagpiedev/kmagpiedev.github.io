...
const CAMERA_KEYFRAMES = Object.freeze([
  { time: 0,  eye: [0.0, 0.10, 10.60], target: [0.0, 0.05, 0.0] },
  { time: 7,  eye: [0.15, 0.16, 9.25], target: [0.0, 0.05, 0.0] },
  { time: 16, eye: [-0.78, 0.48, 8.15], target: [-0.25, 0.15, 0.0] },
  { time: 25, eye: [1.40, 0.62, 8.35], target: [-0.75, 0.30, 0.0] },
  { time: 34, eye: [-0.65, 0.50, 7.15], target: [-0.85, 0.45, 0.0] },
  { time: 42, eye: [1.25, 0.42, 7.65], target: [0.40, 0.15, 0.0] },
  { time: 50, eye: [0.25, 0.32, 8.75], target: [0.0, 0.05, 0.0] },
  { time: 57, eye: [0.0, 0.12, 10.35], target: [0.0, 0.02, 0.0] },
  { time: 60, eye: [0.0, 0.10, 10.60], target: [0.0, 0.05, 0.0] },
]);
...
function diamondGeometry() {
  ...
  for (const face of faces) {
    ...
    for (const index of face) {
      positions.set(vertices[index], cursor);
...
class InstancedMorphMesh {
  constructor(gl, program, geometry, data) {
    this.gl = gl;
...
export class ArtworkRenderer {
  constructor(canvas, assets, options = {}) {
    ...
    this.threadMagpie = new InstancedMorphMesh(gl, this.programs.mesh, diamond, assets.threadToMagpie);
    this.magpieBridge = new InstancedMorphMesh(gl, this.programs.mesh, diamond, assets.magpieToBridge);
  ...
  updateCamera(time) {
    ...
    this.eye[1] += subtle;
    mat4Perspective(this.projection, 42 * Math.PI / 180, this.width / this.height, .1, 100);
  ...
  renderBackground(time, state) {
    ...
    gl.uniform1f(this.backgroundUniforms.lotus, state.lotus);
  ...
  renderSceneMeshes(time, state, pointScale) {
    ...
    composeModel(this.modelMagpie, [state.magpieFlight * 1.6, .03 + state.magpieFlap * .06, 0], -.018 + state.magpieFlap * .025, [1,1,1]);
    composeModel(this.modelBridge, [0,0,0], 0, [1,1,1]);
    composeModel(this.modelLotus, [0,0,0], state.lotus * .08, [1,1,1]);
    ...
    if (this.threadMagpie.draw({
      ...
      opacity:state.magpieOpacity,turbulence:(1-state.magpieMorph)*.60+state.rift*.08,
    ...
    if (this.magpieBridge.draw({
      ...
      opacity:state.bridgeOpacity,turbulence:(1-state.bridgeMorph)*.22,flap:0,pulse:state.seal,glow:1.08,
    })) this.drawCalls++;

    if (this.bridgeLotus.draw({
      viewProj:this.viewProjection,model:this.modelLotus,camera:this.eye,time,morph:state.lotusMorph,
      opacity:state.lotusOpacity,turbulence:.04,flap:0,pulse:state.lotus,glow:1.12,
    })) this.drawCalls++;
    ...
    if (this.particles.draw({
      ...
      opacity:.70+.28*Math.max(state.threads,state.magpieMorph,state.bridgeMorph,state.lotus),
    })) this.drawCalls++;
  ...
  composite(time, state, bloomTexture) {
    ...
    gl.uniform1f(this.compositeUniforms.bloomStrength,.62 + state.seal*.28 + state.lotus*.20);
    gl.uniform1f(this.compositeUniforms.exposure,1.08 + state.window*.05 + state.seal*.07);
